...
class Category {
    ...
    create({ name, icon }) {
        return db.query('INSERT INTO categories (name, icon) VALUES ($1, $2) RETURNING *', [ name, icon ]);
    }

    update({ name, icon, id }) {
        return db.query('UPDATE categories SET name = $1, icon = $2 WHERE id = $3', [ name, icon, id ]);
    }
